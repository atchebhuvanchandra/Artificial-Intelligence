...
#After making dictionary to map ingredients from yummly recipes to ingredients in the flavor network, this is to
...
def make_tfidf(arr):
    print('TF-IDF for all the flavours based on counts in the recipes\n')
    print('input, numpy array with flavor counts for each recipe and compounds return numpy array adjusted as tfidf ')
    arr2 = arr.copy()
    N=arr2.shape[0]
    ...
    print(tfidf.shape)
    print(pd.DataFrame(tfidf).head())
    return tfidf


if __name__ == '__main__':
    print('The yummly dataset is being loaded into an object  (yummly.pkl)\n')
    yum = pd.read_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly.pkl')
    #drop duplicates
    ...
    print('Removing low level information like suger free, fat free etc...')
    yum['clean ingredients'] = yum['clean ingredients'].apply(lambda x:[remove_word(word) for word in x])
    yum.to_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_clean.pkl')
    ...
    comp = pd.read_csv(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\comp_info.tsv',index_col=0,sep='\t')
    ingr_comp = pd.read_csv(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\ingr_comp.tsv',sep='\t')
    ingr = pd.read_csv(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\ingr_info.tsv',index_col=0,sep='\t')
    ...
    yum_ingr.to_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_ingr.pkl')
    yum_X.to_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_ingrX.pkl')
    #get flavor profile
    yum_flavor = flavor_profile(yum_X, ingr, comp, ingr_comp)
    print(pd.DataFrame(yum_flavor).head())
    #make tfidf from flavor profile
    yum_tfidf = make_tfidf(yum_flavor)
    #pickle numpy array as dataframes
    pd.DataFrame(yum_flavor).to_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yum_flavor.pkl')
    pd.DataFrame(yum_tfidf).to_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yum_tfidf.pkl')
