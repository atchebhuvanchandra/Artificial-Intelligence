...
def logistic_test(X,y):
    ...
    print('First round logistic regresion:',metrics.accuracy_score(y_test,y_pred))
    #tune parameter C
    crange =[0.01,0.1,1]
    for num in crange:
...
def svm_test(X,y):
    ...
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    print('First round svm:',metrics.accuracy_score(y_test,y_pred))
    #tune parameter C
    crange =[0.01,0.1,1]
    for num in crange:
...
if __name__ == '__main__':
    ...
    yum_clean = pd.read_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_clean.pkl')
