...
def plot_bokeh(df,sublist,filename):
    ...
    source = ColumnDataSource(data=dict(x=tsne[:,0],y=tsne[:,1],cuisine = df_sub['cuisine'], recipe = df_sub['recipeName']))

    hover = HoverTool(tooltips=[ ("cuisine", "@cuisine"), ("recipe", "@recipe")])

    p = figure(plot_width=1000, plot_height=1000, tools=[hover],title="flavor clustering")
...
if __name__ == '__main__':
    yum_ingr = pd.read_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_ingr.pkl')
    yum_ingrX = pd.read_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yummly_ingrX.pkl')
    yum_tfidf = pd.read_pickle(r'C:\Users\atche\Desktop\Study zone\AI\Main_Project\data\yum_tfidf.pkl')
